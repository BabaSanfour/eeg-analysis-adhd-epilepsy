"""Segment-level QC on post-processed (cleaned) EEG."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Segment-level EEG QC after preprocessing.")
    parser.add_argument("--input_dir", required=True, type=Path, help="BIDS root directory with cleaned EEG files.")
    ...
    parser.add_argument("--bids_proc", default=None, help="BIDS processing label (e.g., clean).")
    parser.add_argument("--min_segment_duration", type=float, default=5.0, help="Minimum segment duration to analyze (s).")
    parser.add_argument(
        "--highpass",
        type=float,
        default=None,
        help="Optional high-pass filter cutoff for QC (Hz). Leave unset to skip filtering.",
    )
...
def process_file(
    ...
    try:
        ...
        raw.load_data()
        if getattr(args, "highpass", None) is not None and args.highpass and args.highpass > 0:
...
def compare_pre_post(pre_summary_path: Path, post_summary_path: Path, output_path: Path, logger: object) -> None:
    if not pre_summary_path.exists() or not post_summary_path.exists():
        return
    pre_df = pd.read_csv(pre_summary_path)
    post_df = pd.read_csv(post_summary_path)

    pre_pref = pre_df.add_prefix("pre_").rename(
        columns={"pre_subject_id": "subject_id", "pre_segment_type": "segment_type"}
    )
    post_pref = post_df.add_prefix("post_").rename(
        columns={"post_subject_id": "subject_id", "post_segment_type": "segment_type"}
    )
    merged = pd.merge(pre_pref, post_pref, on=["subject_id", "segment_type"], how="inner")
    for col in post_df.columns:
        if col in {"subject_id", "segment_type"}:
            continue
        pre_col = f"pre_{col}"
        post_col = f"post_{col}"
        if pre_col in merged.columns and post_col in merged.columns:
            merged[f"delta_{col}"] = merged[post_col] - merged[pre_col]
    merged.to_csv(output_path, index=False)
    logger.info("Saved pre/post comparison to %s", output_path)
...
def main() -> None:
    ...
    if aggregated_frames:
        ...
        summary_df.to_csv(post_summary_path, index=False)

    pre_summary_path = args.pre_summary_path or (output_dir / "segment_qc_pre_summary_by_subject_and_type.csv")
    if pre_summary_path and pre_summary_path.exists() and post_summary_path.exists():
        compare_pre_post(
            pre_summary_path,
            post_summary_path,
            output_dir / "segment_qc_pre_post_comparison_by_subject_and_type.csv",
            logger,
        )
