...
def _get_env_path(var_name: str, default: str | None = None) -> str:
    ...
    if value is None:
        raise OSError(f"Required environment variable '{var_name}' is not set.")
    return value
...
def _load_annotations() -> dict[str, tuple[str, ...]]:
    try:
        ...
        logging.getLogger(__name__).warning(
            "Failed to load annotations from %s: %s", _CONFIG_PATH, e
        )
    return {}
...
SENSOR_ARTEFACT_KEYWORDS = tuple(
    _annot_config.get(
        "sensor_artefact_keywords", _annot_config.get("sensor_missing_keywords", ("manquantes",))
    )
)
