...
from .utils.ica import apply_ica_artifact, fit_ica_context
...
class ArtifactCorrectionConfig:
    ...
    # Methods
    eog_method: str | None = "dss"  # 'dss', 'ica', 'blind-dss', None
    ecg_method: str | None = "dss"  # 'dss', 'ica', 'quasiperiodic', None
    emg_method: str | None = "mwf"  # 'mwf', 'wica', 'ica', 'dss', None
...
def run_source_correction(
    ...
    fit_segments: list[tuple[float, float]] | None = None,
    ...
    if artifact_profile:
        ...
        if muscle_load > 0.15:
            LOGGER.info(
                "Significant artifact load detected (%.1f%%). Boosting correction aggression.",
                muscle_load * 100,
            )
    ...
    if condition_name:
        ...
        if not cond_blocks:
            LOGGER.warning(
                "No blocks found for condition %r. Returning original raw.", condition_name
            )
        ...
        LOGGER.info(
            "Created concatenated raw for %r: %.2fs", condition_name, corrected_raw.times[-1]
        )
    else:
    ...
    if fit_segments:
        ...
        if fit_crops:
            ...
            LOGGER.info(
                "Created training raw from %d segments (%.2fs)",
                len(fit_segments),
                raw_fit.times[-1],
            )
        else:
            LOGGER.warning(
                "fit_segments provided but no valid data extracted. Fallback to target data."
            )
    ...
    ica_context: dict[str, Any] | None = None
    ...
    for art_type, method, ica_labels in artifacts:
        ...
        with benchmark_step(f"{art_type}_removal", provenance):
            ...
            if method == "ica":
                ...
                profile = _get_dss_profile(
                    art_type, method, config, float(corrected_raw.info["sfreq"])
                )
                ...
                if art_type == "eog" and method == "dss" and stats.get("skipped"):
                    ...
                    blind_prof = _get_dss_profile(
                        "eog", "blind-dss", config, float(corrected_raw.info["sfreq"])
                    )
                    corrected_raw, stats = _run_dss_artifact(
                        corrected_raw,
                        config,
                ...
                # Specialized fallback for ECG dss
                    ...
                    qp_prof = _get_dss_profile(
                        "ecg", "quasiperiodic", config, float(corrected_raw.info["sfreq"])
                    )
                    corrected_raw, stats = _run_dss_artifact(
                        corrected_raw,
                        config,
                ...
                cleaned, stats = _remove_emg_mwf(
                    corrected_raw, config, bad_segments, raw_fit, output_dir, subject_id
                )
                if cleaned is not None:
                    corrected_raw = cleaned
            elif art_type == "emg" and method == "wica":
                corrected_raw, stats = _remove_emg_wica(
                    corrected_raw, config, bad_segments, raw_fit, output_dir, subject_id
                )
            else:
...
def _extract_bad_segments(raw: mne.io.BaseRaw) -> list[tuple[float, float]]:
    ...
    return [
        (a["onset"], a["duration"]) for a in raw.annotations if a["description"].startswith("BAD_")
    ]
...
def _remove_emg_mwf(
    ...
    output_dir: Path | None = None,
    subject_id: str = "unknown",
) -> tuple[mne.io.BaseRaw | None, dict]:
    ...
    try:
        ...
        if train_condition:
            ...
            if train_blocks:
                ...
                LOGGER.warning(
                    "Train condition %r not found. Fitting on target data.", train_condition
                )
...
def _load_base_provenance(input_path: Path, preproc_root: Path) -> dict | None:
...
def main():
    ...
    # Selection Arguments
    parser.add_argument(
        "--subjects", nargs="+", help="List of specific subjects to process (e.g. sub-001 sub-002)"
    )
    parser.add_argument(
        "--start-from", type=str, help="Start processing from this subject ID (alphabetical)"
    )
    parser.add_argument("--all", action="store_true", help="Process all subjects found in BIDS")
    parser.add_argument(
        "--test", action="store_true", help="Run on a small subset (5 subjects) for testing"
    )
    ...
    parser.add_argument(
        ...
        help="EMG removal method",
    )
    parser.add_argument(
        "--output-desc",
        type=str,
        default="correct",
        help="BIDS desc entity for output (e.g. correctDss, correctIca)",
    )

    parser.add_argument("--condition", type=str, help="Process only specific condition")
    parser.add_argument(
        "--train-condition", type=str, help="Condition to use for model training (e.g. Rest)"
    )
    ...
    if not files:
        LOGGER.error(
            "No Stage 0 FIF files found in %s (pattern: *_desc-base_eeg.fif).", preproc_dir
        )
        sys.exit(1)
    ...
    if args.skip_existing:
        ...
        for input_file in files:
            ...
            try:
                ...
                LOGGER.error(
                    "Failed rebuilding existing correction QC record for %s (%s): %s",
                    sid,
                    input_file.name,
                    exc,
                    exc_info=True,
